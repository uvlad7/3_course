...
public class View implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        textArea.wrapTextProperty().addListener((observable, oldValue, newValue) -> controller.setWrap(newValue));
        fileChooser = new FileChooser();
        fileChooser.getExtensionFilters().addAll(new FileChooser.ExtensionFilter(resources.getString("textFiles"), "*.java"));
    ...
    public void replaceText(String text) {
        int oldLen = textArea.getText().length(), oldPos = textArea.getCaretPosition();
        textArea.replaceText(0, textArea.getText().length(), text);
        textArea.positionCaret(oldPos - oldLen + textArea.getText().length());
    }
