...
bool bSuspendE, bSuspendR, bSuspendT;
...
void TerminateTriangle(HWND hWnd, HMENU hMenu, BOOL * fTerminateE);
...
void WndProc_OnPaint(HWND hWnd)
{
	...
	EnterCriticalSection(&cs); //dwRetCode = WaitForSingleObject(hMutex, INFINITE);
	...
	//{
		...
		EndPaint(hWnd, &ps);
		nWM_PAINT++;
		LeaveCriticalSection(&cs); //ReleaseMutex(hMutex);
	//}
	//		return 0;
}
...
LONG WndProc_OnDestroy(HWND hWnd)
{
	...
	/*int count = 0;
	if (!fTerminateE) {
		hThreadO[count] = hThreadE[0];
		count++;
	}
	if (!fTerminateR) {
		hThreadO[count] = hThreadE[1];
		count++;
	}
	if (!fTerminateT) {
		hThreadO[count] = hThreadE[2];
		count++;
	}*/
	...
	case WAIT_ABANDONED_0:
		...
		break;
	case WAIT_FAILED:
		wsprintf(szRetRes, TEXT("WAIT_FAILED=%d dwRet=%d"), WAIT_FAILED, dwRet);
		MessageBox(NULL, szRetRes, "WaitForMultipleObjects", MB_OK | MB_ICONEXCLAMATION);
		break;
	case WAIT_OBJECT_0:
	...
	//};
	//fTerminateR = TRUE;
	//if (!fTerminateT && !CloseHandle(hThreadE[2]))
	//{
	//	MessageBox(NULL, "CloseHandle  failed",
	//		"PaintEllipse Thread", MB_OK | MB_ICONEXCLAMATION);
	//};
...
unsigned int __stdcall  PaintTriangle(void* hWnd) {
	...
	while (!fTerminateT) {
		...
		yTop = rand() % (rect.top + 1);
		yBottom = rand() % (rect.bottom + 1);
		POINT vertices[] = { {min(xLeft, xRight), max(yTop, yBottom)}, {max(xLeft, xRight), max(yTop, yBottom)}, 
			{min(xLeft, xRight) + rand() % abs(xLeft - xRight), min(yTop, yBottom)} };
...
void SuspendEllipse(HMENU hMenu, bool* bSuspend)
{
	...
	if (!*bSuspend) {
		EnterCriticalSection(&cs);
		if (!fTerminateE && 0xFFFFFFFF == SuspendThread(hThreadE[0]))//or -1 (==0xFFFFFFFF)
		{
	...
	{
		if (!fTerminateE && 0xFFFFFFFF == ResumeThread(hThreadE[0]))
		{
...
void SuspendTriangle(HMENU hMenu, bool* bSuspend)
{
	...
	if (!*bSuspend) {
		EnterCriticalSection(&cs);
		if (!fTerminateT && 0xFFFFFFFF == SuspendThread(hThreadE[2]))//or -1 (==0xFFFFFFFF)
		{
			wsprintf(message, TEXT("SuspendThread Error %ld"), GetLastError());
	...
	{
		if (!fTerminateT && 0xFFFFFFFF == ResumeThread(hThreadE[2]))
		{
			wsprintf(message, TEXT("ResumeThread Error %ld"), GetLastError());
			MessageBox(NULL, message, "PaintTriangle Thread", MB_OK | MB_ICONEXCLAMATION);
			return;
...
void SuspendRectangle(HMENU hMenu, bool* bSuspend)
{
	...
	case WAIT_OBJECT_0:*/
		...
		if (!*bSuspend) {
			EnterCriticalSection(&cs);
			if (!fTerminateR && 0xFFFFFFFF == SuspendThread(hThreadE[1]))
			{
				wsprintf(message, TEXT("SuspendThread Error %ld"), GetLastError());
		...
		{
			if (!fTerminateR && 0xFFFFFFFF == ResumeThread(hThreadE[1]))
			{
				wsprintf(message, TEXT("ResumeThread Error %ld"), GetLastError());
				MessageBox(NULL, message, "PaintRectangle Thread", MB_OK | MB_ICONEXCLAMATION);
				//ReleaseMutex(hMutex);
